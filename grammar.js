...
export default grammar({
  ...
  rules: {
    ...
    lambda_expression: $ => seq(
      $._keyword_fn,
      $.parameter_list,
      optional(seq('->', field('return_type', $._type_identifier))), // FIX: Also added to lambdas if needed
      choice(
        field('body', $.block),
        seq('=>', field('body', $._expression))
      )
    ...
    // FIX: Changed from commaSep($.identifier) to a dedicated parameter node
    parameter_list: $ => seq('(', commaSep($.parameter_declaration), ')'),

    // FIX: New node to handle both typed and untyped parameters
